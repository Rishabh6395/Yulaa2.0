          ...
          <form onSubmit={handleSave} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              {fc.visible('firstName') && (
                <div>
                  ...
                  <input className="input-field" required={fc.required('firstName')} readOnly={!fc.editable('firstName')} value={form.first_name} onChange={e => setForm(f => ({ ...f, first_name: e.target.value }))}/>
                </div>
              )}
              {fc.visible('lastName') && (
                <div>
                  <label className="label">{fc.label('lastName')}{fc.required('lastName') ? ' *' : ''}</label>
                  <input className="input-field" required={fc.required('lastName')} readOnly={!fc.editable('lastName')} value={form.last_name} onChange={e => setForm(f => ({ ...f, last_name: e.target.value }))}/>
                </div>
              )}
            </div>
            {fc.visible('email') && (
            ...
            {fc.visible('phone') && (
              <div>
                ...
                <input className="input-field" required={fc.required('phone')} readOnly={!fc.editable('phone')} value={form.phone} onChange={e => setForm(f => ({ ...f, phone: e.target.value }))} placeholder="+91 00000 00000"/>
              </div>
            )}
