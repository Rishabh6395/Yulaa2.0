...
import { getLeaveTypes, addLeaveType, patchLeaveType } from '@/modules/masters/masters.service';
...
export async function GET(request: Request) {
  try {
    ...
    return Response.json({ leaveTypes: await getLeaveTypes(schoolId, activeOnly) });
...
export async function POST(request: Request) {
  try {
    ...
    return Response.json({ leaveType: await addLeaveType(schoolId, data) }, { status: 201 });
  } catch (err) { return handleError(err); }
}
...
export async function PATCH(request: Request) {
  try {
    ...
    return Response.json({ leaveType: await patchLeaveType(id, data) });
  } catch (err) { return handleError(err); }
}
