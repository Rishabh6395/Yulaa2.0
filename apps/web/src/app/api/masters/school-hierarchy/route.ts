...
import { getSchoolHierarchy, addSchoolHierarchy, patchSchoolHierarchy } from '@/modules/masters/masters.service';
...
export async function GET(request: Request) {
  try {
    ...
    return Response.json({ hierarchy: await getSchoolHierarchy(schoolId, activeOnly) });
...
export async function POST(request: Request) {
  try {
    ...
    return Response.json({ hierarchy: await addSchoolHierarchy(schoolId, data) }, { status: 201 });
  } catch (err) { return handleError(err); }
}
...
export async function PATCH(request: Request) {
  try {
    ...
    return Response.json({ hierarchy: await patchSchoolHierarchy(id, data) });
  } catch (err) { return handleError(err); }
}
