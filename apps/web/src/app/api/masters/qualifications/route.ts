...
import { getQualificationMasters, addQualificationMaster, patchQualificationMaster } from '@/modules/masters/masters.service';
...
export async function POST(request: Request) {
  try {
    ...
    return Response.json({ qualificationMaster: await addQualificationMaster(schoolId, name, sortOrder) }, { status: 201 });
  } catch (err) { return handleError(err); }
}
...
export async function PATCH(request: Request) {
  try {
    ...
    return Response.json({ qualificationMaster: await patchQualificationMaster(id, data) });
  } catch (err) { return handleError(err); }
}
