...
import { getExamTypes, addExamType, patchExamType } from '@/modules/masters/masters.service';
...
export async function GET(request: Request) {
  try {
    ...
    return Response.json({ examTypes: await getExamTypes(schoolId, activeOnly) });
...
export async function POST(request: Request) {
  try {
    ...
    return Response.json({ examType: await addExamType(schoolId, data) }, { status: 201 });
  } catch (err) { return handleError(err); }
}
...
export async function PATCH(request: Request) {
  try {
    ...
    return Response.json({ examType: await patchExamType(id, data) });
  } catch (err) { return handleError(err); }
}
