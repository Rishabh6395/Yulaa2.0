...
import { getAnnouncementTypes, addAnnouncementType, patchAnnouncementType } from '@/modules/masters/masters.service';
...
export async function GET(request: Request) {
  try {
    ...
    return Response.json({ announcementTypes: await getAnnouncementTypes(schoolId, activeOnly) });
...
export async function POST(request: Request) {
  try {
    ...
    return Response.json({ announcementType: await addAnnouncementType(schoolId, data) }, { status: 201 });
  } catch (err) { return handleError(err); }
}
...
export async function PATCH(request: Request) {
  try {
    ...
    return Response.json({ announcementType: await patchAnnouncementType(id, data) });
  } catch (err) { return handleError(err); }
}
