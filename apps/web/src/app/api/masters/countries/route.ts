...
export async function GET(request: Request) {
  try {
    ...
    return Response.json({ countries: await getCountries(schoolId, activeOnly) });
...
export async function POST(request: Request) {
  try {
    ...
    return Response.json({ country: await addCountry(schoolId, name, code, sortOrder) }, { status: 201 });
  } catch (err) { return handleError(err); }
}
