...
export default function SuperAdminFormConfigPage() {
  ...
  const [globalRules,    setGlobalRules]    = useState<Record<string, any>>({});
  ...
  // ── Load global label overrides for the active form ────────────────────────

  const loadGlobalRules = useCallback(() => {
    if (!defaultSchoolId) return;
    fetch(`/api/form-config?schoolId=${defaultSchoolId}&formId=${activeForm}`, { headers: { Authorization: `Bearer ${token}` } })
      .then(r => r.json())
      .then(d => setGlobalRules(d.configs?.[activeForm]?.['global'] ?? {}))
      .catch(() => {/* silent — global overrides are optional */});
  }, [defaultSchoolId, activeForm]);

  useEffect(() => { loadGlobalRules(); }, [loadGlobalRules]);

  async function saveFieldLabel(fieldId: string, newLabel: string) {
    setLabelSaving(true); setLabelError('');
    try {
      const trimmed = newLabel.trim();
      const updated = { ...globalRules };
      if (trimmed) {
        updated[fieldId] = { ...(updated[fieldId] ?? {}), label: trimmed };
      } else {
        const { label: _removed, ...rest } = updated[fieldId] ?? {};
        if (Object.keys(rest).length) updated[fieldId] = rest;
        else delete updated[fieldId];
      }
      const res = await fetch('/api/form-config', {
        method: 'POST', headers: hdrs,
        body: JSON.stringify({ schoolId: defaultSchoolId, formId: activeForm, role: 'global', fieldRules: updated }),
      });
      const d = await res.json();
      if (!res.ok) throw new Error(d.error || 'Failed to save');
      setGlobalRules(updated);
      setEditingFieldId(null);
    } catch (e: any) { setLabelError(e.message); }
    ...
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="flex gap-6">
        ...
        <div className="w-52 shrink-0 space-y-1">
          {FORMS.map(f => {
            ...
            return (
              <button
                key={f.id}
                onClick={() => { setActiveForm(f.id); setShowAdd(false); setEditingCtId(null); setFieldError(''); setEditingFieldId(null); setLabelError(''); }}
        ...
        <div className="flex-1 min-w-0 space-y-4">
          ...
          {/* Standard fields — renameable */}
          <div className="card p-5 space-y-3">
            <div>
              <h2 className="font-semibold text-gray-900 dark:text-gray-100 flex items-center">
                {form.label}
                <ConfigHelp text="Standard fields are built into the system. Click the pencil icon to rename a field — the new label will appear in all forms. Both the original and renamed labels are shown here. Use 'Push to All Schools' to propagate renames." />
              </h2>
              <p className="text-xs text-surface-400 mt-0.5">
                Standard built-in fields — hover a row to rename
              </p>
            </div>

            {labelError && (
              <p className="text-xs text-red-500 dark:text-red-400 px-1">{labelError}</p>
            )}

            <div className="space-y-0.5">
              {form.fields.map(field => {
                const customLabel = globalRules[field.id]?.label as string | undefined;
                const isEditing   = editingFieldId === field.id;
                return (
                  <div key={field.id}
                    className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg bg-surface-50 dark:bg-gray-800/40 group"
                  >
                    ...
                    {isEditing ? (
                      <div className="flex-1 flex items-center gap-2">
                        <input
                          autoFocus
                          className="flex-1 text-sm border border-brand-300 dark:border-brand-700 rounded-lg px-2.5 py-1 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-brand-400"
                          value={editedLabel}
                          onChange={e => setEditedLabel(e.target.value)}
                          placeholder={field.label}
                          onKeyDown={e => {
                            if (e.key === 'Enter')  saveFieldLabel(field.id, editedLabel);
                            if (e.key === 'Escape') { setEditingFieldId(null); setLabelError(''); }
                          }}
                        />
                        <button
                          onClick={() => saveFieldLabel(field.id, editedLabel)}
                          disabled={labelSaving}
                          className="text-xs px-2.5 py-1 rounded-lg bg-brand-600 hover:bg-brand-700 text-white transition-colors disabled:opacity-50"
                        >
                          {labelSaving ? '…' : 'Save'}
                        </button>
                        <button
                          onClick={() => { setEditingFieldId(null); setLabelError(''); }}
                          className="text-xs px-2 py-1 rounded-lg border border-surface-200 dark:border-gray-700 text-surface-400 hover:bg-surface-50 dark:hover:bg-gray-800 transition-colors"
                        >
                          ✕
                        </button>
                      </div>
                      ...
                      <>
                        <div className="flex-1 flex items-center gap-2 min-w-0">
                          {customLabel ? (
                            <>
                              ...
                              <span className="text-sm font-medium text-gray-900 dark:text-gray-100">{customLabel}</span>
                              <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-brand-100 dark:bg-brand-900/40 text-brand-600 dark:text-brand-400 border border-brand-200 dark:border-brand-800 shrink-0">
                                renamed
                              </span>
                            </>
                          ) : (
                        ...
                        <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                          <button
                            onClick={() => { setEditingFieldId(field.id); setEditedLabel(customLabel ?? ''); setLabelError(''); }}
                            title="Rename field"
                    ...
                    {!isEditing && (
                      <>
                        <span className="text-[10px] text-surface-400 dark:text-gray-500 shrink-0">
                          {TYPE_LABEL[field.type] ?? field.type}
                        </span>
                        {field.fromMaster && (
                          <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700 flex items-center gap-0.5 shrink-0">
