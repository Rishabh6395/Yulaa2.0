...
export default function StatesClient({ schoolId: urlSchoolId }: { schoolId: string }) {
  ...
  const schoolId = urlSchoolId || pickedSchoolId || (() => {
    try { return getStoredUser().schoolId || ''; } catch { return ''; }
  })();
  ...
  const loadStates = useCallback(async (countryId: string) => {
    if (!countryId || !schoolId) return;
    setLoading(true);
    const res = await fetch(`/api/masters/states?countryId=${countryId}&includeInactive=true${schoolQ}`, { headers: authHeaders() });
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    const res = await fetch('/api/masters/states', {
      ...
      body: JSON.stringify({ schoolId, countryId: selectedCountry, ...form }),
    });
    const d = await res.json();
    setSaving(false);
  ...
  async function toggleActive(row: any) {
    await fetch('/api/masters/states', { method: 'PATCH', headers: authHeaders(), body: JSON.stringify({ id: row.id, isActive: !row.isActive }) });
    loadStates(selectedCountry);
  }
      ...
      {isSuperAdmin && !urlSchoolId && (
        <div className="card p-5">
          ...
          {schools.length === 0 ? (
            ...
            <select value={pickedSchoolId} onChange={e => { setPickedSchoolId(e.target.value); setSelectedCountry(''); setRows([]); }} className={sel}>
            ...
            <form onSubmit={handleAdd} className="space-y-3">
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 max-w-lg">
                ...
                <div>
                  ...
                  <input value={form.code} onChange={e => setForm(p => ({...p, code: e.target.value}))} placeholder="e.g. MH" className={inp} />
                </div>
                <div>
              ...
              <button type="submit" disabled={saving || !form.name} className="btn-primary text-sm px-4 py-2">{saving ? 'Saving…' : 'Add State'}</button>
            </form>
          </div>
