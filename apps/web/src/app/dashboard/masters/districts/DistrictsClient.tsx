...
const sel      = 'border border-surface-200 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-900 text-sm px-3 py-2 w-64 focus:outline-none focus:ring-2 focus:ring-blue-500';

export default function DistrictsClient({ schoolId: urlSchoolId }: { schoolId: string }) {
  ...
  useEffect(() => {
    ...
    setIsSuperAdmin(sa);
    if (sa && !urlSchoolId) {
      fetch('/api/super-admin/schools', { headers: authHeaders() })
        .then(r => r.json())
        .then(d => setSchools(d.schools ?? []));
    }
  }, [urlSchoolId]);

  const schoolId = urlSchoolId || pickedSchoolId || getStoredUser().schoolId || '';
  const schoolQ  = schoolId ? `&schoolId=${schoolId}` : '';
  ...
  useEffect(() => {
    ...
    setSelectedState('');
    setRows([]);
    fetch(`/api/masters/countries?includeInactive=true${schoolQ}`, { headers: authHeaders() })
      .then(r => r.json())
  ...
  useEffect(() => {
    ...
    setSelectedState('');
    setRows([]);
    if (!selectedCountry || !schoolId) return;
    fetch(`/api/masters/states?countryId=${selectedCountry}&includeInactive=true${schoolQ}`, { headers: authHeaders() })
      .then(r => r.json())
      .then(d => setStates(d.states ?? []));
  }, [selectedCountry, schoolId, schoolQ]);

  const loadDistricts = useCallback(async (stateId: string) => {
    if (!stateId || !schoolId) return;
    setLoading(true);
    const res = await fetch(`/api/masters/districts?stateId=${stateId}&includeInactive=true${schoolQ}`, { headers: authHeaders() });
    const d   = await res.json();
    setRows(d.districts ?? []);
    setLoading(false);
  }, [schoolId, schoolQ]);

  useEffect(() => { setRows([]); loadDistricts(selectedState); }, [selectedState, loadDistricts]);

  async function handleAdd(e: React.FormEvent) {
    e.preventDefault(); setSaving(true); setErr('');
    const res = await fetch('/api/masters/districts', {
      method: 'POST', headers: authHeaders(),
      body: JSON.stringify({ schoolId, stateId: selectedState, ...form }),
    });
    const d = await res.json();
    setSaving(false);
  ...
  async function toggleActive(row: any) {
    await fetch('/api/masters/districts', { method: 'PATCH', headers: authHeaders(), body: JSON.stringify({ id: row.id, isActive: !row.isActive }) });
    loadDistricts(selectedState);
  }
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-surface-400 mt-0.5">Country → State → District</p>
        </div>
      </div>

      {/* School picker — super_admin with no URL schoolId */}
      {isSuperAdmin && !urlSchoolId && (
        <div className="card p-5">
          ...
          {schools.length === 0 ? (
            ...
            <select value={pickedSchoolId} onChange={e => { setPickedSchoolId(e.target.value); setSelectedCountry(''); setSelectedState(''); setRows([]); }} className={sel}>
      ...
      {schoolId && (
        <div className="card p-5 space-y-4">
          <div>
            <label className="block text-xs font-medium text-surface-400 mb-1">Step 1 — Select Country</label>
            <select value={selectedCountry} onChange={e => setSelectedCountry(e.target.value)} className={sel}>
          ...
          {selectedCountry && (
            <div>
              <label className="block text-xs font-medium text-surface-400 mb-1">Step 2 — Select State</label>
              {states.length === 0 ? (
                <p className="text-xs text-surface-400">No states found for this country. Add states first.</p>
              ) : (
                <select value={selectedState} onChange={e => setSelectedState(e.target.value)} className={sel}>
        ...
        <>
          <div className="card p-5">
            ...
            <form onSubmit={handleAdd} className="space-y-3">
              ...
              <button type="submit" disabled={saving || !form.name} className="btn-primary text-sm px-4 py-2">{saving ? 'Saving…' : 'Add District'}</button>
            </form>
          </div>
          ...
          <div className="card overflow-hidden">
            {loading ? (
              ...
              <div className="p-8 text-center text-sm text-surface-400">No districts yet for this state.</div>
            ) : (
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-surface-100 dark:border-gray-800 bg-surface-50 dark:bg-gray-900/50">
                    {['Name', 'Sort', 'Status', ''].map(h => (
                ...
                <tbody className="divide-y divide-surface-100 dark:divide-gray-800">
                  {rows.map(row => editId === row.id ? (
                    ...
                    <tr key={row.id} className="hover:bg-surface-50 dark:hover:bg-gray-800/50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-surface-500 dark:text-gray-400">{row.sortOrder ?? '—'}</td>
