...
export async function login({
  ...
  loginContext = loginContext ?? "parent";

  const user = await prisma.user.findFirst({
    where: {
      username: username,
      OR: [{ password: password }, { password: encryptedPassword }],
      active: true,
    },
    ...
    if (consultant) {
      ...
      if (!contract) {
        throw new ForbiddenError(
          "No contract found for your account. Contact the school administrator."
        );
      }
