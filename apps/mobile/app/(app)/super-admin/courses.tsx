...
import { getSuperAdminCourses } from '../../../src/api/client';
import { SkeletonCard } from '../../../src/components/ui/Skeleton';
import { COLORS, FONTS, RADIUS, SHADOW } from '../../../src/theme';

const STATUSES = ['all', 'pending', 'approved', 'rejected'];

export default function SuperAdminCoursesScreen() {
  const router = useRouter();
  const [status, setStatus] = useState('all');
  const [refreshing, setRefreshing] = useState(false);

  const { data, isLoading, refetch } = useQuery({
    queryKey: ['super-courses', status],
    queryFn: () => getSuperAdminCourses(status !== 'all' ? `status=${status}` : undefined),
  });
  const onRefresh = useCallback(async () => { setRefreshing(true); await refetch(); setRefreshing(false); }, [refetch]);

  const courses: any[] = data?.courses ?? data?.data ?? (Array.isArray(data) ? data : []);
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      <MotiView from={{ opacity: 0, translateY: -16 }} animate={{ opacity: 1, translateY: 0 }} transition={{ type: 'spring', delay: 0, damping: 18 }} style={styles.topBar}>
        ...
        <Text style={styles.screenTitle}>Course Approvals</Text>
        <View style={{ width: 60 }} />
      </MotiView>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.filtersRow}>
        {STATUSES.map(s => (
          <TouchableOpacity key={s} style={[styles.chip, status === s && styles.chipActive]} onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); setStatus(s); }}>
            <Text style={[styles.chipText, status === s && { color: COLORS.white }]}>{s === 'all' ? 'All' : s.charAt(0).toUpperCase() + s.slice(1)}</Text>
          </TouchableOpacity>
        ))}
      </ScrollView>
            ...
            <MotiView from={{ opacity: 0, translateY: 10 }} animate={{ opacity: 1, translateY: 0 }} transition={{ type: 'spring', delay: index * 40, damping: 18 }}>
              <View style={styles.card}>
                <View style={styles.cardRow}>
                  ...
                  <View style={{ flex: 1 }}>
                    <Text style={styles.courseName}>{item.title ?? item.name ?? 'Course'}</Text>
                    {item.vendor_name && <Text style={styles.meta}>🏪 {item.vendor_name}</Text>}
                    {item.category && <Text style={styles.meta}>📂 {item.category}</Text>}
                    {item.created_at && <Text style={styles.meta}>📅 {new Date(item.created_at).toLocaleDateString()}</Text>}
                  </View>
                  {item.status && (
...
const styles = StyleSheet.create({
  ...
  courseName:    { ...FONTS.bold, fontSize: 15, color: COLORS.text },
  ...
  badgeApproved: { backgroundColor: '#10b98122' },
  badgeRejected: { backgroundColor: '#ef444422' },
  badgePending:  { backgroundColor: '#f59e0b22' },
