...
import { getSuperAdminVendors } from '../../../src/api/client';
import { SkeletonCard } from '../../../src/components/ui/Skeleton';
import { COLORS, FONTS, RADIUS, SHADOW } from '../../../src/theme';

const STATUSES = ['all', 'active', 'pending', 'suspended'];

export default function SuperAdminVendorsScreen() {
  const router = useRouter();
  const [search, setSearch]       = useState('');
  const [status, setStatus]       = useState('all');
  const [refreshing, setRefreshing] = useState(false);

  const { data, isLoading, refetch } = useQuery({ queryKey: ['super-vendors', status], queryFn: () => getSuperAdminVendors(status !== 'all' ? `status=${status}` : undefined) });
  const onRefresh = useCallback(async () => { setRefreshing(true); await refetch(); setRefreshing(false); }, [refetch]);

  const vendors: any[] = data?.vendors ?? data?.data ?? [];
  const filtered = vendors.filter((v: any) => !search || (v.name ?? '').toLowerCase().includes(search.toLowerCase()));
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      <MotiView from={{ opacity: 0, translateY: -16 }} animate={{ opacity: 1, translateY: 0 }} transition={{ type: 'spring', delay: 0, damping: 18 }} style={styles.topBar}>
        ...
        <Text style={styles.screenTitle}>All Vendors</Text>
        <View style={{ width: 60 }} />
      </MotiView>
            ...
            <MotiView from={{ opacity: 0, translateY: 10 }} animate={{ opacity: 1, translateY: 0 }} transition={{ type: 'spring', delay: index * 40, damping: 18 }}>
              <View style={styles.card}>
                <View style={styles.cardRow}>
                  ...
                  <View style={{ flex: 1 }}>
                    <Text style={styles.vendorName}>{item.name ?? item.business_name ?? 'Vendor'}</Text>
...
const styles = StyleSheet.create({
  ...
  category:     { ...FONTS.regular, fontSize: 13, color: COLORS.textMuted, marginTop: 2 },
